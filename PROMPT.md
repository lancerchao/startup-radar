...
3. **Select 2–3 candidates** scoring highest on: (a) quantified evidence of pain, (b) a sharp "why now, not 2 years ago," (c) why incumbents won't or can't, (d) a reachable distribution channel, (e) a first sellable version buildable in weeks, not years. **Any B2C candidate must additionally pass all 8 gates in `B2C_RULES.md` before selection** — score it against each gate explicitly in the digest; fail one gate and reject it outright rather than including it as a maybe. Also apply the file's "good to knows" section to how a passing candidate gets probed. B2B candidates aren't subject to this filter.
